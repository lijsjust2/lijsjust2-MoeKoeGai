    ...
    <div class="player-container">
        ...
        <div class="player-bar">
            ...
            <div class="extra-controls">
                ...
                <button class="extra-btn" :title="t('xia-zai')" @click="handleDownload" :disabled="downloading">
...
import { getCover, share, downloadWithMetadata } from '../utils/utils';
...
const handleDownload = async () => {
    if (downloading.value || !currentSong.value || !currentSong.value.hash) {
    ...
    try {
        ...
        try {
            ...
            if (savedSettings) {
                ...
                console.log('[PlayerControl] 从localStorage获取的settings:', settings);
                if (settings.downloadQuality) {
                    console.log('[PlayerControl] 获取到的downloadQuality:', settings.downloadQuality);
                    // 从新的设置格式映射到音质值
                    switch (settings.downloadQuality) {
                        case 'high':
                            ...
                            break;
                        case 'lossless':
                            defaultQuality = 'flac'; // 使用flac作为无损音质
                            break;
                        case 'hires':
                            defaultQuality = 'hires'; // 使用hires作为Hi-Res音质
                            break;
                        case 'normal':
                        default:
                            defaultQuality = '128';
            ...
            console.warn('读取设置失败，使用默认音质:', error);
        }
        
        console.log('[PlayerControl] 最终使用的音质:', defaultQuality);
        ...
        console.log('[PlayerControl] 开始获取下载链接:', data);
       // 获取下载链接
        let response;
        let fallbackQuality = defaultQuality;
        
        try {
            response = await get('/song/url', data);
            console.log('[PlayerControl] 获取下载链接响应:', response);
            
            // 详细日志，帮助诊断问题
            console.log('[PlayerControl] 响应状态:', response.status);
            console.log('[PlayerControl] 响应结构:', {
                hasUrl: !!response.url,
                hasData: !!response.data,
                hasDataData: response.data?.data ? true : false,
                responseKeys: Object.keys(response)
            });
        } catch (error) {
            console.error('[PlayerControl] 下载链接获取失败:', error);
            // 如果是 Hi-Res 音质请求失败，尝试回退到无损音质
            if (defaultQuality === 'hires') {
                console.log('[PlayerControl] Hi-Res 音质请求失败，尝试回退到无损音质');
                fallbackQuality = 'flac';
                const fallbackData = {
                    hash: currentSong.value.hash,
                    quality: fallbackQuality
                };
                if (!MoeAuth.isAuthenticated) {
                    fallbackData.free_part = 1;
                }
                console.log('[PlayerControl] 开始获取下载链接 (回退):', fallbackData);
                response = await get('/song/url', fallbackData);
                console.log('[PlayerControl] 获取下载链接响应 (回退):', response);
            } else if (defaultQuality === 'flac') {
                // 如果是无损音质请求失败，尝试回退到高品音质
                console.log('[PlayerControl] 无损音质请求失败，尝试回退到高品音质');
                fallbackQuality = '320';
                const fallbackData = {
                    hash: currentSong.value.hash,
                    quality: fallbackQuality
                ...
                try {
                    ...
                    console.error('[PlayerControl] 高品音质请求失败:', error);
                    // 如果高品音质也失败，尝试回退到普通音质
                    console.log('[PlayerControl] 高品音质请求失败，尝试回退到普通音质');
                    fallbackQuality = '128';
                    ...
                    if (!MoeAuth.isAuthenticated) {
                        finalFallbackData.free_part = 1;
                    }
                    console.log('[PlayerControl] 开始获取下载链接 (最终回退):', finalFallbackData);
                    response = await get('/song/url', finalFallbackData);
                    console.log('[PlayerControl] 获取下载链接响应 (最终回退):', response);
                }
            } else if (defaultQuality === '320') {
                // 如果是高品音质请求失败，尝试回退到普通音质
                console.log('[PlayerControl] 高品音质请求失败，尝试回退到普通音质');
                fallbackQuality = '128';
                const fallbackData = {
                    hash: currentSong.value.hash,
        ...
        if (response) {
            console.log('[PlayerControl] 开始解析下载链接，响应:', response);
            
            // 尝试从不同的响应结构中提取下载链接
            if ((response.status === 1 || response.status === 2) && response.url) {
                console.log('[PlayerControl] 检查 response.url 结构:', {
                    type: typeof response.url,
                    isArray: Array.isArray(response.url),
                    length: Array.isArray(response.url) ? response.url.length : 'N/A',
                    firstElement: Array.isArray(response.url) ? response.url[0] : response.url
                });
                
                if (Array.isArray(response.url)) {
                    downloadUrl = response.url[0];
                    console.log('[PlayerControl] 从 response.url 数组提取下载链接:', downloadUrl);
                } else if (typeof response.url === 'string') {
                    downloadUrl = response.url;
                    console.log('[PlayerControl] 从 response.url 字符串提取下载链接:', downloadUrl);
                }
            } else if (response.data && response.data.url) {
                downloadUrl = response.data.url;
                console.log('[PlayerControl] 从 response.data.url 提取下载链接:', downloadUrl);
            } else if (response.data && response.data.data) {
                if (response.data.data.url) {
                    downloadUrl = response.data.data.url;
                    console.log('[PlayerControl] 从 response.data.data.url 提取下载链接:', downloadUrl);
                } else if (Array.isArray(response.data.data) && response.data.data[0]?.url) {
                    downloadUrl = response.data.data[0].url;
                    console.log('[PlayerControl] 从 response.data.data[0].url 提取下载链接:', downloadUrl);
                } else if (response.data.data.list && response.data.data.list[0]?.url) {
                    downloadUrl = response.data.data.list[0].url;
                    console.log('[PlayerControl] 从 response.data.data.list[0].url 提取下载链接:', downloadUrl);
                } else if (response.data.data.audio_data && response.data.data.audio_data.url) {
                    downloadUrl = response.data.data.audio_data.url;
                    console.log('[PlayerControl] 从 response.data.data.audio_data.url 提取下载链接:', downloadUrl);
                }
            }
            
            // 额外的结构检查
            if (!downloadUrl) {
                console.log('[PlayerControl] 尝试其他可能的结构:', {
                    'response.url': response.url,
                    'response.data': response.data,
                    'response.data.data': response.data?.data,
                    'response.songurl': response.songurl,
                    'response.songinfo': response.songinfo
                });
                
                // 尝试其他可能的结构
                if (response.songurl && Array.isArray(response.songurl)) {
                    downloadUrl = response.songurl[0];
                    console.log('[PlayerControl] 从 response.songurl 提取下载链接:', downloadUrl);
                } else if (response.songinfo && response.songinfo.url) {
                    downloadUrl = response.songinfo.url;
                    console.log('[PlayerControl] 从 response.songinfo.url 提取下载链接:', downloadUrl);
                }
            }
        }
        ...
        console.log('[PlayerControl] 获取到下载链接:', downloadUrl);
        
        downloadUrl = downloadUrl.replace('http://', 'https://');
        
        console.log('[PlayerControl] 开始下载歌曲:', downloadUrl);
        ...
        try {
            ...
            const downloadOptions = {
                ...
                onProgress: (message, progressValue) => {
                    console.log(`[PlayerControl] 下载进度: ${progressValue}% - ${message}`);
                }
            };
            ...
            if (result.success) {
                ...
                console.log('[PlayerControl] 歌曲下载已触发:', result.fileName);
                window.$modal.alert(t('xia-zai-yi-chu-fa'));
            } else {
                throw new Error(result.error || '下载失败');
            }
        } catch (error) {
            console.error('增强版下载失败:', error);
            // 如果增强版下载失败，尝试使用原始方式
            const fileExtension = downloadUrl.includes('.flac') ? 'flac' : 'mp3';
            const fileName = `${currentSong.value.name} - ${currentSong.value.author}.${fileExtension}`;
            ...
            console.log('[PlayerControl] 歌曲下载已触发(备用方式):', safeFileName);
            window.$modal.alert(t('xia-zai-yi-chu-fa'));
        }
