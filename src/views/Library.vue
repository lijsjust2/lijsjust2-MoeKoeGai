                ...
                <div class="profile-info">
                    ...
                    <div class="user-details">
                        ...
                        <div class="user-actions">
                            ...
                            <span class="action-button" @click="getVip">领取VIP</span>
                            <span class="action-button" @click="upgradeVip">升级VIP</span>
                        </div>
                        <div class="vip-status-section" v-if="vipStatus || vipRecord">
                            <div class="vip-status-row">
                                <!-- 检查是否有激活的VIP -->
                                <div class="vip-status-item">
                                    <span class="vip-label">VIP状态：</span>
                                    <span class="vip-value" :class="{ active: hasActiveVip() }">{{ hasActiveVip() ? '已激活' : '未激活' }}</span>
                                </div>
                                
                                <!-- 显示VIP有效期 -->
                                <div class="vip-status-item" v-if="getVipEndDate()">
                                    <span class="vip-label">有效期至：</span>
                                    <span class="vip-value">{{ getVipEndDate() }}</span>
                                </div>
                            </div>
                            
                            <!-- 显示每种VIP的信息 -->
                            <div v-if="vipStatus && typeof vipStatus === 'object' && vipStatus.busi_vip && Array.isArray(vipStatus.busi_vip) && vipStatus.busi_vip.length > 0">
                                <div class="vip-status-item" v-for="(vip, index) in vipStatus.busi_vip" :key="index" v-if="vip && vip.is_vip === 1">
                                    <span class="vip-label">{{ getVipTypeName(vip.product_type) }}：</span>
                                    <span class="vip-value">{{ vip.vip_begin_time || '' }} 至 {{ vip.vip_end_time || '' }}</span>
                                </div>
                            </div>
                            
                            <!-- 显示当月已领天数 -->
                            <div class="vip-status-item" v-if="vipRecord && (vipRecord.total_days || vipRecord.days)">
                                <span class="vip-label">当月已领：</span>
                                <span class="vip-value">{{ vipRecord.total_days || vipRecord.days }}天</span>
                            </div>
                        </div>
                    </div>
                </div>
...
// VIP 相关状态
const vipStatus = ref({}); // VIP 状态
const vipRecord = ref({}); // 当月已领取 VIP 天数
...
const getUserDetails = () => {
    // 获取用户详细信息
    getUserDetail().catch(error => {
        console.warn('获取用户详细信息失败:', error.message);
    });
    // 获取用户听歌历史
    getlisten().catch(error => {
        console.warn('获取用户听歌历史失败:', error.message);
    }).finally(() => {
        isLoading.value = false; 
    });
    // 获取用户创建和收藏的歌单
    getplaylist().catch(error => {
        console.warn('获取用户歌单失败:', error.message);
    });
    // 获取用户关注的歌手
    getfollow().catch(error => {
        console.warn('获取用户关注的歌手失败:', error.message);
    });
...
const getVipInfo = async () => {
    try {
        ...
        if (VipInfoResponse.status === 1) {
            ...
            console.warn('获取用户VIP信息失败，接口返回状态:', VipInfoResponse.status);
            // 即使失败，也要继续加载其他信息
            getUserDetails();
        }
    } catch (error) {
        console.warn('获取用户VIP信息失败:', error.message);
        // 即使失败，也要继续加载其他信息
        getUserDetails();
    }
}
...
const getVipStatus = async () => {
    try {
        const statusResponse = await get('/youth/union/vip');
        console.log('VIP 状态接口响应:', statusResponse);
        if (statusResponse.status === 1) {
            vipStatus.value = statusResponse.data;
            console.log('VIP 状态:', vipStatus.value);
            console.log('VIP 状态值:', vipStatus.value.vip_status);
        } else {
            console.warn('获取 VIP 状态失败，接口返回状态:', statusResponse.status);
            // 即使失败，也要确保vipStatus是一个对象，避免页面渲染错误
            vipStatus.value = {};
        }
    } catch (error) {
        console.warn('获取 VIP 状态失败:', error.message);
        console.warn('错误详情:', error);
        // 发生错误时，确保vipStatus是一个对象，避免页面渲染错误
        vipStatus.value = {};
    }
}

// 获取当月已领取 VIP 天数
const getVipRecord = async () => {
    try {
        const recordResponse = await get('/youth/month/vip/record');
        console.log('VIP 记录接口响应:', recordResponse);
        if (recordResponse.status === 1) {
            vipRecord.value = recordResponse.data;
            console.log('VIP 领取记录:', vipRecord.value);
        } else {
            console.warn('获取 VIP 领取记录失败，接口返回状态:', recordResponse.status);
            // 即使失败，也要确保vipRecord是一个对象，避免页面渲染错误
            vipRecord.value = {};
        }
    } catch (error) {
        console.warn('获取 VIP 领取记录失败:', error.message);
        console.warn('错误详情:', error);
        // 发生错误时，确保vipRecord是一个对象，避免页面渲染错误
        vipRecord.value = {};
    }
}
...
const upgradeVip = async () => {
    try {
        ...
        if (upgradeResponse.status === 1) {
            window.$modal.alert('升级成功，已升级为畅听 VIP');
            // 重新获取 VIP 状态
            await getVipStatus();
            await getVipRecord();
        }
    } catch (error) {
        window.$modal.alert('升级失败，请先领取一天 VIP');
    }
}
...
const hasActiveVip = () => {
    try {
        if (vipStatus.value && typeof vipStatus.value === 'object' && vipStatus.value.is_vip === 1) {
            return true;
        }
        if (vipStatus.value && typeof vipStatus.value === 'object' && vipStatus.value.busi_vip && Array.isArray(vipStatus.value.busi_vip)) {
            return vipStatus.value.busi_vip.some(vip => vip && vip.is_vip === 1);
        }
        return false;
    } catch (error) {
        console.warn('检查VIP状态时出错:', error.message);
        return false;
    }
}

// 获取VIP类型的中文名称
const getVipTypeName = (productType) => {
    try {
        const typeMap = {
            'svip': '超级VIP',
            'tvip': '畅听VIP',
        ...
        return typeMap[productType] || productType || '未知VIP';
    } catch (error) {
        console.warn('获取VIP类型名称时出错:', error.message);
        return '未知VIP';
    }
}
...
const getVipEndDate = () => {
    try {
        ...
        // 检查主账户的VIP结束时间
        if (vipStatus.value && typeof vipStatus.value === 'object' && (vipStatus.value.vip_end_time || vipStatus.value.end_time)) {
            endDate = vipStatus.value.vip_end_time || vipStatus.value.end_time;
        }
        
        // 检查busi_vip数组中的VIP结束时间，取最晚的一个
        if (vipStatus.value && typeof vipStatus.value === 'object' && vipStatus.value.busi_vip && Array.isArray(vipStatus.value.busi_vip)) {
            for (const vip of vipStatus.value.busi_vip) {
                if (vip && vip.is_vip === 1 && vip.vip_end_time) {
                    if (!endDate || new Date(vip.vip_end_time) > new Date(endDate)) {
                        endDate = vip.vip_end_time;
                    }
                }
            }
        }
        ...
        console.warn('获取VIP有效期时出错:', error.message);
        return null;
    }
}
