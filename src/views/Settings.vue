...
const settingSections = computed(() => [
    ...
    },
    {
        ...
        items: [
            ...
            {
                key: 'downloadQuality',
                label: t('mo-ren-xia-zai-yin-zhi'),
                icon: '⬇️ '
            },
            {
...
const getItemIcon = (key) => {
    const iconMap = {
        ...
        'batchAlbumDownload': 'fas fa-download',
...
const selectionTypeMap = {
    ...
    downloadQuality: {
        title: t('mo-ren-xia-zai-yin-zhi'),
        options: [
            { displayText: t('pu-tong-yin-zhi'), value: 'normal' },
            { displayText: t('gao-yin-zhi-320kbps'), value: 'high' },
            { displayText: t('wu-sun-yin-zhi-1104kbps'), value: 'lossless' },
...
const selectOption = (option) => {
    ...
    const actions = {
        ...
        'downloadQuality': () => {
            ...
            if (!MoeAuth.isAuthenticated && option.value !== 'normal') {
                window.$modal.alert(t('gao-pin-zhi-xia-zai-xu-yao-deng-lu-hou-cai-neng-shi-yong'));
            }
        }
    };
    ...
    const refreshHintTypes = ['nativeTitleBar','lyricsBackground', 'lyricsFontSize', 'gpuAcceleration', 'highDpi', 'apiMode', 'touchBar', 'preventAppSuspension', 'networkMode', 'font', 'proxy', 'dataSource', 'loudnessNormalization', 'statusBarLyrics', 'downloadQuality'];
