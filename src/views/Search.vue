                ...
                <ul v-if="searchType === 'song'">
                    <li v-for="(result, index) in searchResults" :key="index" class="result-item"
                        ...
                        <div class="result-actions">
                            <button class="action-btn download-btn" @click.stop="downloadSong(result)" title="下载">
...
const downloadSong = async (song) => {
    try {
        const hash = song?.HQFileHash || song?.SQFileHash || song?.FileHash;
        if (!hash) {
            console.error('无法获取歌曲 hash');
            return;
        }
        ...
        if (downloadUrl) {
            ...
            if (result.success) {
                ...
                link.href = blobUrl;
                link.download = result.fileName || `${song.OriSongName} - ${song.SingerName}.${quality === 'flac' ? 'flac' : 'mp3'}`;
                link.target = '_blank';
